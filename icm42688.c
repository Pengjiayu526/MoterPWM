...
/* 延时函数由 delay.h/delay.c 提供，此处不再重复定义 */
...
static uint8_t icm42688_read_reg(uint8_t reg)
{
    ...
    SPI_CS(1);
#elif defined(ICM_USE_I2C)
    if (I2C_ReadReg(ICM42688_ADDRESS, reg, &regval, 1U) != I2C_RESULT_OK)
    {
        return 0U;
    }
#endif
...
*******************************************************************************/
static int8_t icm42688_read_regs(uint8_t reg, uint8_t* buf, uint16_t len)
{
#if defined(ICM_USE_HARD_SPI)
    reg |= 0x80;
    SPI_CS(0);
	...
	if ((buf == (uint8_t *)0) ||
	    (len == 0U) ||
	    (len > I2C_RX_MAX_PACKET_SIZE))
	{
		return -1;
	}

	if (I2C_ReadReg(ICM42688_ADDRESS, reg, buf, (uint8_t)len) != I2C_RESULT_OK)
	{
		return -1;
	}
...
static uint8_t icm42688_write_reg(uint8_t reg, uint8_t value)
{
	...
	{
		I2C_Result_t result;
		result = I2C_WriteReg(ICM42688_ADDRESS, reg, &value, 1U);
		return (result == I2C_RESULT_OK) ? 0U : 1U;
	}
#endif
    return 0;
}
...
int8_t bsp_IcmGetTemperature(int16_t* pTemp)
{
    ...
    if (icm42688_read_regs(ICM42688_TEMP_DATA1, buffer, 2) != 0)
    {
        return -1;
    }
...
int8_t bsp_IcmGetAccelerometer(icm42688RawData_t* accData)
{
    ...
    if (icm42688_read_regs(ICM42688_ACCEL_DATA_X1, buffer, 6) != 0)
    {
        return -1;
    }
...
int8_t bsp_IcmGetGyroscope(icm42688RawData_t* GyroData)
{
    ...
    if (icm42688_read_regs(ICM42688_GYRO_DATA_X1, buffer, 6) != 0)
    {
        return -1;
    }
...
int8_t bsp_IcmGetRawData(icm42688RealData_t* accData, icm42688RealData_t* GyroData)
{
    ...
    if (icm42688_read_regs(ICM42688_ACCEL_DATA_X1, buffer, 12U) != 0)
    {
        return -1;
    }
